...
from src.prompt.prompt import cluster_validation_prompt, synthesis_prompt
...
def synthesize_stories(validation_results, max_retries = 6):
    results = []
        ...
        while retries < max_retries:
            try:
                ...
                try:
                    ...
                    synthesis = {"error": "Failed to parse JSON", "raw": response.text}

                # --- save result ---
                with open(f"synthesis/synthesis_{count}.json", "w", encoding="utf-8") as f:
                    json.dump(synthesis, f, ensure_ascii=False, indent=2)

                print(f"Cluster {count} synthesized.")
                ...
                retries += 1
                wait = min(60, 2 ** retries + random.random())
                print(f"Error synthesizing cluster {count} (attempt {retries}): {e}. Retrying in {wait:.1f}s...")
                time.sleep(wait)
        else:
            # ran out of retries
            fail_data = {"error": f"Failed after {max_retries} retries"}
            results.append(fail_data)
            with open(f"synthesis/synthesis_{count}_failed.json", "w", encoding="utf-8") as f:
                json.dump(fail_data, f, ensure_ascii=False, indent=2)
